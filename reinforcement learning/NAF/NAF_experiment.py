...
if __name__ == "__main__":
    ...
    parser.add_argument('--hidden_activation', dest='hidden_activation', nargs='+',
                        default=tf.nn.relu)  # TODO parse string to set TF activation function object
    ...
    parser.add_argument('--covariance', dest='covariance', type=str, nargs='+',
                        default="original")  # covariance matrix for NAF algorithm (original, diagonal, identity, (not tested) square)
    parser.add_argument('--solve_threshold', dest='solve_threshold', type=float,
                        default=None)  # threshold for having solved environment - different thresholds for different environments in same experiment not supported
    ...
    def experiment(args):
        ...
        for i in range(args['repeats']):
            agent = naf.Agent(args['v'], env.observation_space, env.action_space, args['learning_rate'],
                              args['batch_normalize'], args['gamma'], args['tau'], args['epsilon'], args['hidden_size'],
                              args['hidden_n'], args['hidden_activation'], args['batch_size'], args['memory_capacity'],
                              args['load_path'], args['covariance'])
            experiment_rewards = []
            terminate = None
                ...
                for k in range(args['max_episode_steps']):
                    ...
                    if np.isnan(np.min(action)):  # if NaN action (neural network exploded)
                        ...
                        with open("error.txt", "a") as error_file:
                            error_file.write(
                                str(args) + " repeat " + str(i) + " episode " + str(j) + " step " + str(k) + " NaN\n")
                        rewards = 0
                        terminate = "nan"
                        break
                    # print(action)
                    state_next, reward, terminal, _ = env.step(
                        agent.scale(action, env.action_space.low, env.action_space.high))
    ...
    # keys = ['v', 'graph', 'render', 'environment', 'repeats', 'episodes', 'max_episode_steps', 'train_steps',
    #         'batch_normalize', 'learning_rate', 'gamma', 'tau', 'epsilon', 'hidden_size', 'hidden_n', 'hidden_activation',
    #         'batch_size', 'memory_capacity', 'load_path', 'covariance', 'solve_threshold']
    # vals = [args.v, args.graph, args.render, args.environment, args.repeats, args.episodes, args.max_episode_steps,
    #         args.train_steps, args.batch_normalize, args.learning_rate, args.gamma, args.tau, args.epsilon,
    #         args.hidden_size, args.hidden_n, args.hidden_activation, args.batch_size, args.memory_capacity, args.load_path,
    #         args.covariance, args.solve_threshold]
    #
    # # run experiments
