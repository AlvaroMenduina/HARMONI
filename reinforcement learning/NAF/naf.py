    ...
    def __init__(self, input_layer, out_n, activation=None, batch_normalize=False):
        ...
        if batch_normalize:
            ...
            with tf.control_dependencies([update_mean, update_var]):
                x = tf.nn.batch_normalization(x, self.moving_mean, self.moving_var, self.beta, self.gamma,
                                              variance_epsilon)
...
class Agent:
    def __init__(self, v, observation_space, action_space, learning_rate, batch_normalize, gamma, tau, epsilon,
                 hidden_size, hidden_n, hidden_activation, batch_size, memory_capacity, load_path, covariance):
        self.v = v
        ...
        self.t_H = NeuralNetworkLayer(self.x, H_n, activation=hidden_activation,
                                      batch_normalize=batch_normalize)  # target
        ...
        for i in range(1, H_layer_n):
            ...
            self.t_H = NeuralNetworkLayer(self.t_H.h, H_n, activation=hidden_activation,
                                          batch_normalize=batch_normalize)  # target
            self.updates += self.t_H.construct_update(self.H, self.tau)
        ...
        if covariance == "identity":  # identity covariance
            ...
            self.O = NeuralNetworkLayer(self.H.h, mu_n,
                                        batch_normalize=batch_normalize)  # nn input to diagonal covariance
            self.P = tf.matrix_set_diag(tf.eye(self.action_n, batch_shape=[tf.shape(self.x)[0]]),
                                        self.O.h)  # diagonal covariance

        elif covariance == "square":  # square (nonzero) covariance with nn inputs (every entry in matrix is unique input from nn) NOT TESTED
            self.O = NeuralNetworkLayer(self.H.n.h, mu_n * mu_n,
                                        batch_normalize=batch_normalize)  # nn inputs to square covariance
            self.P = tf.reshape(self.O, [mu_n, mu_n])

        else:  # original NAF covariance
            self.M = NeuralNetworkLayer(self.H.h, M_n, activation=tf.nn.tanh,
                                        batch_normalize=batch_normalize)  # tanh activation function to avoid exploding gradient TODO instead normalise rewards?
        ...
        self.A = (-1.0 / 2.0) * tf.reshape(tf.matmul(tf.matmul(self.D, self.P), tf.transpose(self.D, perm=[0, 2, 1])),
    ...
    def update_noise(self):
        if self.memory.ready:
            ...
            self.epsilon = 1.0 / (1.0 + 0.1 * i + (1.0 / (i + 1)) * np.log(
                i))  # derived through black magic for inverted double pendulum
    ...
    def get_target(self, a, r, s_next, terminal):
        targets = np.reshape(r, [-1, 1]) + np.reshape(
            self.gamma * self.sess.run(self.t_V.h, feed_dict={self.x: s_next, self.u: a}), [-1, 1])
    ...
    def backprop(self, batch_state, batch_action, batch_target):
        l, a, _ = self.sess.run([self.loss, self.A, self.optimiser],
                                feed_dict={self.x: batch_state, self.target: batch_target, self.u: batch_action})
        return l, a
